...
NEW_KEYS = [
    "viewProfilePicture", "selectProfilePicture", "showLess", "showMore", 
    "profilePicture", "updateProfilePicture", "fileSizeError", 
    "profilePictureUpdated", "profilePictureUpdateFailed", 
    "profilePictureUploadError", "newProfilePicture", "selectPhoto", 
    "maxFileSize", "supportedFormats", "error", "success"
]
...
def main():
    ...
    profile_keys = tr_data.get('profile', {})
    ...
    for filename in json_files:
        ...
        for key in NEW_KEYS:
            source_value = profile_keys.get(key)
            if not source_value: 
                continue
                
            current_value = data['profile'].get(key)
            if not current_value or current_value == source_value:
                print(f"   📝 profile.{key} çevriliyor...")
                translated = translate_text(source_value, lang_code)
                data['profile'][key] = translated
                print(f"      → {translated}")
                modified = True
                time.sleep(0.5)
