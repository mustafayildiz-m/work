...
def main():
    ...
    notif_keys = tr_data.get('notifications', {})
    if not notif_keys:
        print("❌ 'notifications' anahtarı bulunamadı.")
        return

    print(f"🔍 Toplam {len(notif_keys)} bildirim anahtarı bulundu.")
    ...
    for filename in json_files:
        ...
        for key, source_value in notif_keys.items():
            current_value = data['notifications'].get(key)
            if not current_value or current_value == source_value:
                print(f"   📝 notifications.{key} çevriliyor...")
                translated = translate_text(source_value, lang_code)
                data['notifications'][key] = translated
                print(f"      → {translated}")
                modified = True
                time.sleep(0.5)
            else:
