...
def main():
    ...
    for filename in json_files:
        ...
        for key, source_value in source_section.items():
            ...
            # Re-translate if current value is 'Sil', or looks like Turkish (not translated properly)
            # Or if it doesn't exist.
            if not current_value or current_value == "Sil" or current_value == source_value:
                # Except if we are translating to English and we have "Decline" or "Reject"
                if lang_code == "en" and current_value in ["Decline", "Reject", "Delete"]:
                   # Check if it matches our preference for Decline
                   if current_value != "Decline":
                       print(f"   📝 {section_name}.{key} (EN'e güncelleniyor)...")
                       data[section_name][key] = "Decline"
                       modified = True
                   continue
                   
                print(f"   📝 {section_name}.{key} çevriliyor...")
