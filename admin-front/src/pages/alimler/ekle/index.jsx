...
const AddScholarPage = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${BOOKS_URL}?limit=1000`, { headers })
      .then(res => {
        if (res.status === 401) {
          toast.error(intl.formatMessage({ id: "UI.OTURUM_SURESI_DOLDU" }));
          return { data: [] };
        }
        return res.json();
      })
  ...
  const handleSubmit = async e => {
    ...
    const token = localStorage.getItem('access_token');
    if (!token) {
      toast.error(intl.formatMessage({ id: "UI.LUTFEN_GIRIS_YAPIN" }));
      setLoading(false);
      return;
    }
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="flex flex-col">
              ...
              <input name="fullName" value={form.fullName} onChange={handleChange} required placeholder={intl.formatMessage({ id: "UI.ADI_SOYAD" })} className="px-3 py-2 border rounded bg-white dark:bg-gray-800" />
            </div>
            <div className="flex flex-col">
          ...
          <div className="mt-4">
            ...
            {form.ownBooks.map((book, idx) => (
              <div key={book.tempId || `ownbook-${idx}`} className="flex flex-col gap-2 mb-2 bg-gray-50 dark:bg-gray-800 p-3 rounded">
                <input value={book.title} onChange={e => handleOwnBookChange(idx, 'title', e.target.value)} placeholder={intl.formatMessage({ id: "UI.KITAP" })} className="px-2 py-1 border rounded bg-white dark:bg-gray-800" />
