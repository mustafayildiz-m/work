  ...
  implements MigrationInterface
{
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    try {
      ...
      console.log(
        'Foreign key FK_711de461c8fcc1a9ff6864142dc does not exist, skipping drop',
      );
    }
    ...
    try {
      await queryRunner.query(
        `CREATE TABLE \`scholar_stories\` (\`id\` int NOT NULL AUTO_INCREMENT, \`title\` varchar(255) NOT NULL, \`description\` text NULL, \`video_url\` varchar(500) NULL, \`thumbnail_url\` varchar(500) NULL, \`duration\` int NULL, \`language\` varchar(50) NOT NULL DEFAULT 'tr', \`is_active\` tinyint NOT NULL DEFAULT 1, \`is_featured\` tinyint NOT NULL DEFAULT 0, \`view_count\` int NOT NULL DEFAULT '0', \`like_count\` int NOT NULL DEFAULT '0', \`scholar_id\` int NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
      );
    } catch (error) {
    ...
    try {
      await queryRunner.query(
        `ALTER TABLE \`books\` ADD \`summary\` text NULL`,
      );
    } catch (error) {
    ...
    try {
      await queryRunner.query(
        `ALTER TABLE \`scholars\` DROP COLUMN \`biography\``,
      );
    } catch (error) {
    ...
    try {
      await queryRunner.query(
        `ALTER TABLE \`scholars\` ADD \`biography\` text NOT NULL`,
      );
    } catch (error) {
    ...
    try {
      ...
      // Foreign key doesn't exist, continue
      console.log(
        'Foreign key FK_805a09bc0444e439b53dfc0b061 does not exist, skipping drop',
      );
    }
    ...
    try {
      ...
      console.log(
        'Could not change book_languages.languageId, column may not exist or type already correct',
      );
    }
    try {
      await queryRunner.query(
      ...
      console.log(
        'Could not change book_languages.bookId, column may not exist or type already correct',
      );
    }
    ...
    try {
      ...
      console.log(
        'Foreign key FK_711de461c8fcc1a9ff6864142dc already exists, skipping add',
      );
    }
    try {
      await queryRunner.query(
      ...
      console.log(
        'Foreign key FK_805a09bc0444e439b53dfc0b061 already exists, skipping add',
      );
    }
    try {
      await queryRunner.query(
        `ALTER TABLE \`scholar_stories\` ADD CONSTRAINT \`FK_91ce4af53b8aa6f13f65bf2c6f5\` FOREIGN KEY (\`scholar_id\`) REFERENCES \`scholars\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`,
      );
    } catch (error) {
      console.log(
        'Foreign key FK_91ce4af53b8aa6f13f65bf2c6f5 already exists, skipping add',
      );
    }
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE \`scholar_stories\` DROP FOREIGN KEY \`FK_91ce4af53b8aa6f13f65bf2c6f5\``,
    );
    await queryRunner.query(
      `ALTER TABLE \`book_languages\` DROP FOREIGN KEY \`FK_805a09bc0444e439b53dfc0b061\``,
    );
    ...
    await queryRunner.query(
      `ALTER TABLE \`scholars\` ADD \`biography\` longtext NOT NULL`,
    );
