...
export class SeedPodcasts1730730000000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const sampleAudioUrl =
      '/uploads/1762287977518-9d6c07b2-c8fd-4170-806b-34c5d5c2f9fd.mp3';
    const sampleCoverImage =
      '/uploads/1762288119484-8aa12c1a-cec3-4ae0-a467-c9ed066997d5.jpg';

    // Türkçe podcastler (20 adet)
    for (let i = 0; i < 20; i++) {
      podcasts.push({
        title:
          turkishTitles[i % turkishTitles.length] +
          (i > 14 ? ` - Bölüm ${i - 14}` : ''),
    ...
    for (let i = 0; i < 15; i++) {
      podcasts.push({
        title:
          englishTitles[i % englishTitles.length] +
          (i > 14 ? ` - Part ${i - 14}` : ''),
    ...
    for (let i = 0; i < 12; i++) {
      podcasts.push({
        title:
          arabicTitles[i % arabicTitles.length] +
          (i > 14 ? ` - ${i - 14} جزء` : ''),
    ...
    for (let i = 0; i < 8; i++) {
      podcasts.push({
        title:
          germanTitles[i % germanTitles.length] +
          (i > 7 ? ` - Teil ${i - 7}` : ''),
    ...
    for (let i = 0; i < 7; i++) {
      podcasts.push({
        title:
          frenchTitles[i % frenchTitles.length] +
          (i > 6 ? ` - Partie ${i - 6}` : ''),
        description:
          "Comprendre l'Islam à travers des sujets variés et enrichissants.",
        audioUrl: sampleAudioUrl,
        coverImage: sampleCoverImage,
    ...
    for (let i = 0; i < 5; i++) {
      podcasts.push({
        title:
          japaneseTitles[i % japaneseTitles.length] +
          (i > 4 ? ` - 第${i - 4}部` : ''),
    ...
    for (let i = 0; i < 3; i++) {
      podcasts.push({
        title:
          russianTitles[i % russianTitles.length] +
          (i > 2 ? ` - Часть ${i - 2}` : ''),
